...
with tabs[3]:
    ...
    df = df.rename(columns={
        "projectsecretaris_betrokken": "Projectsecretaris betrokken",
        "projectsecretaris": "Projectsecretaris"
    })
    ...
    if not df.empty and IS_EDITOR:
        ...
        with st.form("project_edit_form"):
            ...
            projectsecretaris_betrokken = st.selectbox(
                "Projectsecretaris betrokken?",
                ["Nee", "Ja"],
                index=1 if project["projectsecretaris_betrokken"] == "Ja" else 0
            )
        ...
        if totaal_taken > 0:
            percentage = round((afgerond_taken / totaal_taken) * 100)
        ...
        df_taken = pd.read_sql(
            ...
            ORDER BY
                CASE status
                    WHEN 'Actief' THEN 1
                    WHEN 'Niet gestart' THEN 2
                    WHEN 'Wachten op' THEN 3
                    WHEN 'Afgerond' THEN 4
                END,
                einddatum
            """,
            c,
        ...
        if df_taken.empty:
            ...
            for _, taak in df_taken.iterrows():
                ...
                with st.container(border=True):
                    ...
                    if (
                        pd.notna(taak["omschrijving"])
                        and taak["omschrijving"]
                    ):
                    ...
                    if taak["status"] == "Afgerond":
                        ...
                        st.info("In uitvoering")
                    elif taak["status"] == "Wachten op":
                        st.warning("Wachten op")
                    else:
                        st.error("Nog niet gestart")
        ...
        if not df_taken.empty:
            ...
            with st.form("taak_bewerken"):
                ...
                prioriteit = st.selectbox(
                    "Prioriteit",
                    ["Laag", "Gemiddeld", "Hoog"],
                    index=["Laag", "Gemiddeld", "Hoog"].index(
                        taak["prioriteit"]
                    )
                )
                ...
                opslaan = col1.form_submit_button(
                    "💾 Opslaan"
                )

                verwijderen = col2.form_submit_button(
                    "🗑️ Verwijderen"
                )
                ...
                if opslaan:
                    ...
                    if status == "Afgerond":
                        ...
                        voltooid_op = date.today().isoformat()
        ...
        with st.form("taak_toevoegen", clear_on_submit=True):
            ...
            toevoegen = st.form_submit_button(
                "➕ Taak toevoegen",
                use_container_width=True
            )
    ...
    if not df.empty and IS_EDITOR:
        ...
        st.warning(
            "⚠️ Dit project wordt definitief verwijderd."
        )

        bevestiging = st.checkbox(
            "Ik weet zeker dat ik dit project wil verwijderen",
            key="bevestig_project"
        )
...
with tabs[4]:
    ...
    if not df_werk.empty:
        ...
        geselecteerde_label = st.selectbox(
            "Selecteer werkzaamheid",
            list(werk_opties.keys()),
            key="werkzaamheden_kaart_select"
        )
        geselecteerde_id = werk_opties[geselecteerde_label]
        ...
        if actieve_id not in werk_opties.values():
            ...
            st.session_state.pop(
                "werkzaamheden_kaart_selected_id", None
            )

        if actieve_id is None:
            st.info(
                "Selecteer een werkzaamheid en klik op "
                "‘📍 Werkzaamheid selecteren’ om de kaart te tonen."
            )
        else:
            geselecteerde = df_werk[
                df_werk["id"] == actieve_id
            ].iloc[0]

            st.success(
                f"📍 Geselecteerd: **{geselecteerde['titel']}**"
            )

            try:
                lat = (
                    float(geselecteerde["latitude"])
                    if pd.notna(geselecteerde["latitude"])
                    else 51.8133
                )
                lon = (
                    float(geselecteerde["longitude"])
                    if pd.notna(geselecteerde["longitude"])
                    else 4.6901
                )
            except Exception:
                lat, lon = 51.8133, 4.6901

            # Exact dezelfde Leaflet/OpenStreetMap-kaart voor alle rollen.
            m = folium.Map(
                location=[lat, lon],
                zoom_start=15,
                control_scale=True
            ...
            for _, r in df_werk.iterrows():
                is_selected = int(r["id"]) == int(actieve_id)
            ...
            if can_edit_work:
                ...
                # Viewer: dezelfde kaart, zonder tekenlaag.
                st_folium(
                    m,
                    width=1200,
                    height=700,
            ...
            # Alleen admin/editor mogen beoordelen, tekenen, opslaan en verwijderen.
            if can_edit_work:
                st.divider()
                ...
                huidige_status = geselecteerde.get(
                    "status_parkeren"
                )
                if (
                ...
                behandeld_door = st.text_input(
                    "Behandeld door",
                    value=(
                        geselecteerde["behandeld_door"]
                        if pd.notna(geselecteerde["behandeld_door"])
                        else ""
                    ),
                    key="werk_behandeld"
                )
                ...
                opmerking = st.text_area(
                    "Opmerking",
                    value=(
                        geselecteerde["opmerking_parkeren"]
                        if pd.notna(geselecteerde["opmerking_parkeren"])
                        else ""
                    ),
                    key="werk_opmerking"
                )
                ...
                if st.button(
                    ...
                    if (
                        ...
                        st.success("✅ Werkgebied opgeslagen")
                        st.rerun()
                    else:
                        st.warning(
                ...
                if bevestiging and st.button(
                    ...
                    st.success(
                        f"✅ Verwijderd: {geselecteerde_label}"
                    )
                    st.session_state.pop(
                        "werkzaamheden_kaart_selected_id",
                        None
